...
const PALETTES: Record<
  Theme,
  { accent: string; fill: string; grid: string; muted: string; surface: string; text: string }
> = {
  saffron: {
    accent: "#e8a43a",
...
export function ReportSpeedChart({
  durationMs,
  peakSpeed,
  speedUnit,
  theme,
  timeline,
}: ReportSpeedChartProps) {
          ...
          data: [
            ...
            ...timeline.map((sample) => ({
              ...
              y:
                speedUnit === "cpm"
                  ? sample.burstCpm
                  : (sample.burstWpm ?? Math.round(sample.burstCpm / 5)),
            })),
          ],
