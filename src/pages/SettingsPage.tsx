  ...
  return (
    <section className="mx-auto w-[min(880px,100%)] animate-[arrive_.4s_ease_both]">
      <div className="mb-[35px] flex items-end justify-between border-b border-app-line pb-[22px]">
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-[.13em] text-app-accent">សម្រួលឱ្យស្របនឹងអ្នក</p>
      ...
      <div className="grid grid-cols-2 gap-2.5 max-[760px]:grid-cols-1">
        <div className={settingCardClass}>
          <div className={settingLeadClass}>
            ...
            <div className={settingCopyClass}><h2>ឯកតាល្បឿន</h2><p>CPM សម្រាប់ចង្កោមខ្មែរ ឬ WPM ស្តង់ដារ ៥ តួអក្សរ</p></div>
          </div>
        ...
        <div className={settingCardClass}>
          <div className={settingLeadClass}>
            <span className={settingIconClass}><SunIcon /></span>
            <div className={settingCopyClass}><h2>រូបរាង</h2><p>ពណ៌ស្រទន់សម្រាប់ការផ្តោតអារម្មណ៍</p></div>
          </div>
          <div className="grid shrink-0 gap-1">
        ...
        <div className={settingCardClass}>
          <div className={settingLeadClass}>
            ...
            <div className={settingCopyClass}><h2>ទំហំអក្សរ</h2><p>រក្សារូបរាងចង្កោមឱ្យច្បាស់</p></div>
          </div>
          <label className="flex w-[140px] shrink-0 items-center gap-[9px]">
            <input className="w-[95px] accent-app-accent" type="range" min="38" max="64" value={settings.fontSize} onChange={(event) => update("fontSize", Number(event.target.value))} />
            <output className="text-xs text-app-accent [font-variant-numeric:tabular-nums]">{settings.fontSize}px</output>
          </label>
        </div>
        ...
        <div className={settingCardClass}>
          <div className={settingLeadClass}>
            <span className={settingIconClass}>↕</span>
            <div className={settingCopyClass}><h2>គម្លាតបន្ទាត់</h2><p>បន្ថែមកន្លែងសម្រាប់ស្រៈលើ និងក្រោម</p></div>
          </div>
          <label className="flex w-[140px] shrink-0 items-center gap-[9px]">
            <input className="w-[95px] accent-app-accent" type="range" min="1.5" max="2.2" step="0.05" value={settings.lineHeight} onChange={(event) => update("lineHeight", Number(event.target.value))} />
            <output className="text-xs text-app-accent [font-variant-numeric:tabular-nums]">{settings.lineHeight.toFixed(2)}</output>
          </label>
        </div>

        <div className={settingCardClass}>
          <div className={settingLeadClass}>
            <span className={settingIconClass}><SoundIcon /></span>
            <div className={settingCopyClass}><h2>សំឡេង</h2><p>សញ្ញាស្រាលពេលបញ្ចប់ចង្កោមត្រឹមត្រូវ</p></div>
          </div>
          <button className="group relative h-[22px] w-[38px] shrink-0 cursor-pointer rounded-xl bg-app-hover p-[3px] transition-colors data-[on=true]:bg-app-accent-soft" data-on={settings.sound} onClick={() => update("sound", !settings.sound)} aria-pressed={settings.sound} aria-label="Typing sound">
        ...
        <div className={settingCardClass}>
          <div className={settingLeadClass}>
            <span className={settingIconClass}>។</span>
            <div className={settingCopyClass}><h2>សញ្ញាវណ្ណយុត្ត</h2><p>បន្ថែមសញ្ញាខ្មែរទៅក្នុងលំហាត់</p></div>
          </div>
          <button className="group relative h-[22px] w-[38px] shrink-0 cursor-pointer rounded-xl bg-app-hover p-[3px] transition-colors data-[on=true]:bg-app-accent-soft" data-on={settings.punctuation} onClick={() => update("punctuation", !settings.punctuation)} aria-pressed={settings.punctuation} aria-label="Khmer punctuation">
            <span className="block size-4 rounded-full bg-app-dim transition-[transform,background] group-data-[on=true]:translate-x-4 group-data-[on=true]:bg-app-accent" />
          </button>
        </div>
        ...
        <div className={settingCardClass}>
          <div className={settingLeadClass}>
            <span className={settingIconClass}><RestartIcon /></span>
            <div className={settingCopyClass}><h2>ចាប់ផ្ដើមឡើងវិញ</h2><p>ប្រើផ្លូវកាត់ពីអេក្រង់សាកល្បង</p></div>
          </div>
