...
export function TypingPage({
  ...
  const pendingKey = session.typing.pendingInput
    ? khmerTextEngine.canonicalize(session.typing.pendingInput)
    : "";
      ...
      {!session.result && (
        <>
          ...
          <div
            ...
            data-error={session.typing.pendingStatus === "incorrect"}
          >
            <div
              ref={surfaceRef}
            ...
            >
              <div className="select-none text-pretty font-khmer text-[length:var(--prompt-size)] font-normal leading-[var(--prompt-leading)] tracking-[.012em] text-app-dim [word-spacing:.12em] max-[760px]:text-[min(var(--prompt-size),42px)]">
                {session.typing.prompt.map((cluster, index) => {
                  ...
                  const incorrect =
                    clusterState === "incorrect" ||
                    (active && session.typing.pendingStatus === "incorrect");
                  return (
                    <span
                      ...
                      className={cx(
                        ...
                        incorrect &&
                          "text-app-error underline decoration-[color-mix(in_srgb,var(--error)_65%,transparent)] decoration-2 underline-offset-[.2em]",
                        active && "[text-shadow:0_0_24px_var(--accent-soft)]",
                        active &&
                          session.typing.pendingInput &&
                          session.typing.pendingStatus === "prefix" &&
                          "text-[color-mix(in_srgb,var(--accent)_72%,var(--text-soft))] underline decoration-[color-mix(in_srgb,var(--accent)_38%,transparent)] decoration-2 decoration-dotted underline-offset-[.2em]",
                      )}
                      data-cluster={index}
                ...
                <span
                  ...
                  aria-label={
                    session.typing.pendingStatus === "incorrect"
                      ? `Incorrect input: ${session.typing.pendingInput}`
                      : `Matching cluster prefix: ${session.typing.pendingInput}`
                  }
                  style={
                    {
            ...
            >
              ...
              {session.typing.startedAt === null
                ...
                : session.typing.pendingStatus === "incorrect"
                  ? "អក្សរនេះមិនត្រូវគ្នា — លុបដើម្បីកែ"
                  : "កំពុងវាយជាចង្កោមអក្សរខ្មែរ"}
            </div>
          </div>
